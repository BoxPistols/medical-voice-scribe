...
| 方向 | 場所 |
| --- | --- |
| アプリ → Storybook | フッターの「デザインシステム」 |
| Storybook → アプリ | サイドバー左上のブランド名、および「はじめに / 概要」のボタン |

URLは `src/lib/siteLinks.ts`(アプリ側)と `.storybook/manager.ts`(Storybook側)に持つ。
どちらも環境変数で上書きできる。デプロイ先を変えるときはこの2箇所を見る。
