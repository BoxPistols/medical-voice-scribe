...
export const PROVIDERS: Record<ProviderKey, ProviderConfig> = {
  ...
  gemini: {
    ...
    // 登録するのは、日本語と英語の両方のドキュメントに載っているモデルだけにする。
    //
    // APIにはgemini-3.7-flashとgemini-3.8-flashも存在し、実際に呼べる。
    // ただしai.google.dev/gemini-api/docs/models?hl=jaには未掲載で、英語版にしか無い。
    // 加えて3.8はversionが "3.0" を返す(3.6は"3.6-flash-07-2026"、3.7は"3.7-flash-08-2026")。
    // 無料枠の混雑も3.8が最も激しかった(2026-09-06実測, 各5回で3.8が3/5、3.7が4/5、3.6が5/5)。
    //
    // 3.6は日英どちらにも載り、版番号も整い、価格は3.8と同額($0.75/$3.75)。
    // 日本語ドキュメントが追いついたら、そのとき新しい方へ上げる。
